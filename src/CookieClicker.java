...
public class CookieClicker {
    ...
    private static final int POP_UP_SPEED = 3;
    ...
    public static void startTutorial() {
        ...
        Label welcomeMessage = new Label("<html>Welcome to our game: "
            + "Cookie Clicker.<br><br>Your goal "
            +
            "is to collect as many cookies as possible. "
            +
            "Click on the falling cookies to collect them."
            +
            "<br>Use them to buy upgrades that will help you to collect "
            +
            "your cookies even faster.<br><br>Created by "
            +
            "Marnix van den Bosch and Alex Aichimoaie.</html>", Color.RED, 0);
        ...
        startButton.setLocation(
            (frame.getWidth() - startButton.getWidth()) / 2,
            (frame.getHeight() - startButton.getHeight()) / 2
        );
        ...
        Timer pulseTimer = new Timer(50, new ActionListener() {
            private final float scaleSpeed = 0.02f;
    ...
    private static void runGame() {
        ...
        Label label = new Label("<html>Cookie count: 0<br>CPS: 0</html>", Color.BLACK, 255);
        label.setBounds(0, 0, 300, 40);
        panel.add(label);

        Label label2 = new Label("<html>Milk Storm: 0<br>Milk: 0</html>", Color.BLACK, 255);
        ...
        Label costCursor = new Label(cursorUpgrade.getCost() + " Cookies", Color.BLACK, 255);
        costCursor.setBounds(250, 100, 150, 40);
        storePanel.add(costCursor);

        Label multiplierCursor = new Label("x2", Color.BLACK, 255);
        ...
        Label grandmaLabel = new Label("0", Color.BLACK, 255);
        grandmaLabel.setBounds(250, 250, 40, 40);
        storePanel.add(grandmaLabel);

        Label costGrandmaLabel = new Label(grandma.getCost() + " Cookies", Color.BLACK, 255);
        costGrandmaLabel.setBounds(250, 300, 150, 40);
        storePanel.add(costGrandmaLabel);

        Label cpsGrandmaLabel = new Label(grandma.getGrandmaCPS() + " CPS", Color.BLACK, 255);
        cpsGrandmaLabel.setBounds(250, 350, 150, 40);
        ...
        Label costFactoryLabel = new Label(factory.getCost() + " Cookies", Color.BLACK, 255);
        costFactoryLabel.setBounds(250, 500, 150, 40);
        storePanel.add(costFactoryLabel);

        Label cpsFactoryLabel = new Label(factory.getFactoryCPS() + " CPS", Color.BLACK, 255);
        cpsFactoryLabel.setBounds(250, 550, 150, 40);
        ...
        CPS cps = new CPS(0);
        CountDown time = new CountDown(MINI_GAME_TIME);

        ActionListener upgradeListener = e -> {
            if ((counter.getCookie() >= cursorUpgrade.getCost())
                && (!cursorUpgrade.getUpgraded())) { // requirements for upgrade
                counter.setCookie(counter.getCookie() - cursorUpgrade.getCost()); //cost
                counter.setIncrement(counter.getIncrement() * 2); //upgrade
                label.setText(
                    "<html>Cookie count: " + counter.getCookie() 
                    +
                        "<br>CPS: " + cps.getCps() 
                        +
                            "</html>"
                ...
                label2.setText(String.format(
                    "<html>Milk Storm: %d<br>Milk: %d</html>",
                    time.getTime(),
                    milk.getMilkCount()
                ));
            ...
            } else if ((milk.getMilkCount() >= cursorUpgrade.getMilkCost())
                && cursorMilkUpgradable
                && !cursorMilkUpgraded
                && cursorUpgrade.getUpgraded()) {
                ...
                label2.setText(
                    "<html>Milk Storm: " + time.getTime() 
                    +
                    "<br>Milk: " + milk.getMilkCount() 
                    +
                    "</html>"
                );
            ...
            } else if ((counter.getCookie() < cursorUpgrade.getCost())
                && !cursorUpgrade.getUpgraded()) {
                ...
                Label upgradeFailedLabel = new Label(
                    "Upgrade already purchased!",
                    Color.BLACK,
                    155
                );
                showPopUp(upgradeFailedLabel, panel, labels);
            } else if (cursorMilkUpgradable
                    && (milk.getMilkCount() < cursorUpgrade.getMilkCost())) {
                Label upgradeFailedLabel = new Label("Not enough milk!", Color.BLACK, 155);
                showPopUp(upgradeFailedLabel, panel, labels);
            } else {
                Label upgradeFailedLabel = new Label(
                    "Upgrade already purchased!",
        ...
        Timer fallTimer = new Timer(20, e -> {
            ...
            if (time.getTime() <= 0 && !miniGameStarted) {
                ...
                game.requestFocusInWindow(); //so that you don't have to click on the window
            }

            if (milkCount != 0) {
                milk.setMilkCount(milk.getMilkCount() + milkCount);
                milkCount = 0;
                costCursor.setText(cursorUpgrade.getMilkCost() + " Milk");
            }

            if (miniGameFinished) {
                miniGameStarted = false;
                miniGameFinished = false;
                time.setTime(MINI_GAME_TIME);
                frame.remove(game);
                frame.add(panel);
                frame.revalidate();
                frame.repaint();
            ...
            for (Label l : labels) {
                l.setLocation(l.getX(), l.getY() - POP_UP_SPEED);
    ...
    //This algorithm detemines the spawns of the cookies 
    //based on the spawn availability in the array.
    private static void spawnCookie(
            ArrayList<Cookie> cookies,
        JPanel panel,
        CookieCounter counter,
        Label label,
            ArrayList<Integer> spawningPoint,
            CPS cps,
            CountDown time,
        Label label2,
        Milk milk
